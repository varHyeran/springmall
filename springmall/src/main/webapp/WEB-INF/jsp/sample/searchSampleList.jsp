...
<head>
<meta charset="UTF-8">
<title>searchSampleList</title>
<!-- bootstrap CDN -->
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.1.3/css/bootstrap.min.css" integrity="sha384-MCw98/SFnGE8fJT3GXwEOngsV7Zt27NXFoaoApmYm81iuXoPkFOJwJ8ERdknLPMO" crossorigin="anonymous">
<!-- jquery CDN -->
</head>
<body>
	<h1 align="center">searchSampleList</h1>
	<table class="table" style="width:60%;height:100px;margin:auto;text-align:center;">	<!-- 검색샘플리스트 -->
		<thead>
			<tr>
				<td></td><td></td><td></td>
				<td>
		...
		<tbody>
		<!-- model안의 searchSampleList 가져와서 사용 -->
			<c:forEach var="sample" items="${searchSampleList}">
				<tr>
		...
		<div class="form-group text-center">
			...
			<a href="/sample/searchSampleList?currentPage=1" class="btn btn-link"></a>
				<a href="/sample/searchSampleList?currentPage=${currentPage-1}" class="btn btn-info">이전</a>
			</c:if>
	        
	        <c:forEach var = "click" begin="${startPage}" end="${endPage}" step="1">
	            <c:if test = "${click!=currentPage}">
	                <a href="/sample/searchSampleList?currentPage=${click}">${click}</a>
	            </c:if>
	            <c:if test = "${click == currentPage}">
	                ${click}
	            </c:if>
			...
			<c:if test="${currentPage < lastPage}">		<!-- 현재페이지가 마지막페이지보다 작다면 다음버튼 생성 -->
				<a href="/sample/searchSampleList?currentPage=${currentPage+1}" class="btn btn-info">다음</a>
				<a href="/sample/searchSampleList?currentPage=${lastPage}" class="btn btn-link"></a>
			</c:if>
		</div>
		<!-- 검색 -->
